...
class UCCLEPHTPrepareAndFinalize(mk.FusedMoEPrepareAndFinalizeModular):
    """
    Prepare/Finalize using UCCL EP High-Throughput kernels.
    """

    @staticmethod
    def maybe_roundup_layer_hidden_size(hidden_size: int, dtype: torch.dtype) -> int:
        # Round up hidden size so it is compatible with UCCL EP High Throughput
        # kernels.
        # UCCL EP intranode kernels make copies in units of,
    ...
    def _do_dispatch(
        ...
        # Capture a UCCL EP event on the compute stream before yielding.
    ...
    def _receiver(
        ...
        #
        # UCCLEP's topk_ids output refers to the local experts directly. Offset
        ...
        #   UCCLEP kernels only support dispatching block scales.
        ...
        if not quant_config.is_block_quantized and not defer_input_quant:
            ...
            if expert_x.numel() != 0:
                ...
                expert_x, expert_x_scale = moe_kernel_quantize_input(
                    ...
                    # as of now, uccl ep does not handle FP4
