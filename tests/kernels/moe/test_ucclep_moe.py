...
"""
Test ucclep dispatch-combine logic, copied from deepep dispatch combine test
"""
...
from vllm.utils.import_utils import has_uccl_ep
...
def make_modular_kernel(
    ...
    ht_args: UCCLEPHTArgs | None = None
    ll_args: UCCLEPLLArgs | None = None

    if low_latency_mode:
        ll_args = UCCLEPLLArgs(
        ...
        ht_args = UCCLEPHTArgs(num_local_experts=num_local_experts)

    a2a: UCCLEPHTPrepareAndFinalize | UCCLEPLLPrepareAndFinalize = make_ucclep_a2a(
        pg=pg,
        pgi=pgi,
        dp_size=dp_size,
        q_dtype=q_dtype,
        block_shape=None,
        ucclep_ht_args=ht_args,
        ucclep_ll_args=ll_args,
    )
...
def _uccl_ep_moe(
    ...
    with set_current_vllm_config(VllmConfig()):
        ...
        ucclep_combined = uccl_ep_moe_impl(
            pg,
            pgi,
            low_latency_mode,
            dp_size,
...
@multi_gpu_test(num_gpus=2)
@requires_uccl_ep
def test_low_latency_uccl_ep_moe(
    dtype: torch.dtype,
    m: int,
    n: int,
    k: int,
    ...
    if low_latency_mode and k not in UCCLEPLLPrepareAndFinalize.SUPPORTED_HIDDEN_SIZES:
        pytest.skip(
            f"Skipping test as hidden size {k} is not in list of supported "
            f"hidden sizes {UCCLEPLLPrepareAndFinalize.SUPPORTED_HIDDEN_SIZES}"
        )
